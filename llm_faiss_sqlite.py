...
# 保存文件路径和总结内容到数据库
def save_metadata_to_db(db_path, file_paths, summarized_contents):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    for (folder_name, file_path), summarized_content in zip(file_paths, summarized_contents):
        cursor.execute('''
        INSERT INTO metadata (folder_name, file_path, summarized_content) VALUES (?, ?, ?)
        ''', (folder_name, file_path, summarized_content))
    conn.commit()
    conn.close()

# 从数据库加载文件路径和总结内容
def load_metadata_from_db(db_path):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute('SELECT folder_name, file_path, summarized_content FROM metadata')
    rows = cursor.fetchall()
    file_paths = [(row[0], row[1]) for row in rows]
    summarized_contents = [row[2] for row in rows]
    conn.close()
    return file_paths, summarized_contents

# 检查是否有新的文件需要处理
def get_processed_files_from_db(db_path):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute('SELECT file_path FROM metadata')
    rows = cursor.fetchall()
    processed_files = [os.path.basename(row[0]) for row in rows]
    conn.close()
...
if not os.path.exists(index_file_path):
    ...
    if len(embeddings) > 0:
        ...
        save_metadata_to_db(db_path, file_paths, summarized_contents)
        print("Index and metadata created and saved.")
else:
    # 如果索引文件存在，则加载索引
    faiss_index = load_faiss_index(index_file_path)
    file_paths, summarized_contents = load_metadata_from_db(db_path)
    print("Index and metadata loaded from file.")
    
    # 处理新的文件
    processed_files = get_processed_files_from_db(db_path)
    for folder_path in folder_paths:
        new_file_paths, new_summarized_contents, new_embeddings = load_and_encode_txt_files(folder_path, processed_files)
        if len(new_embeddings) > 0:
            ...
            save_faiss_index(faiss_index, index_file_path)
            save_metadata_to_db(db_path, new_file_paths, new_summarized_contents)
